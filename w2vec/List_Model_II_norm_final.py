...
from w2vec import MLIST_10TH as m_list
...
m_list_larger_90 = [33, 35, 100, 109, 134, 198, 210, 336, 397, 403, 407, 429, 579, 584, 592, 602, 624, 684, 713, 754, 755, 778, 785, 814, 834, 876, 929, 953, 977, 1016, 1022, 1031, 1071, 1079, 1112, 1139, 1140, 1144, 1147, 1152, 1158, 1209, 1224, 1227, 1238, 1246, 1292, 1332, 1333, 1357, 1385, 1388, 1423, 1472, 1520, 1522, 1524, 1620]
...
def calculate_params():
    ...
    for i in range(len(all_probs)):
        ...
        x_values = np.column_stack((all_concreteness_part_norm, all_wordfreq_part_norm, all_wordlen_part_norm,  all_m_list_part_norm))
...
print(
    "Parameters of List Recall Regression Model II (X-intercept, Conc coeff, WordFreq coeff,WordLen coeff,  MPool 10,  MList 10%)",
    params)
...
#              round(np.mean(beta_mlist_10), 3), round(np.mean(beta_mpool_10), 3)]
# sem_betas = [round(stats.sem(beta_concreteness), 3), round(stats.sem(beta_wordfreq), 3),
#              round(stats.sem(beta_wordlen), 3), round(stats.sem(beta_mlist_10), 3), round(stats.sem(beta_mpool_10), 3)]
ave_betas = [round(np.mean(beta_concreteness), 3), round(np.mean(beta_wordfreq), 3), round(np.mean(beta_wordlen), 3),
             round(np.mean(beta_mlist_10), 3)]
sem_betas = [round(stats.sem(beta_concreteness), 3), round(stats.sem(beta_wordfreq), 3),
             round(stats.sem(beta_wordlen), 3), round(stats.sem(beta_mlist_10), 3)]
predictors = ['Concreteness', 'Word Frequency', 'Word Length', 'List Meaningfulness 10%']
print(ave_betas, ave_betas)

t = Table([predictors, ave_betas, sem_betas], names=("List Recall Model II", "Mean Betas", "SEM Betas"),
