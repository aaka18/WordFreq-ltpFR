...
def calculate_params():
    ...
    for i in range(len(all_probs)):
        ...
        x_values = np.column_stack((all_concreteness_part_norm, all_wordfreq_part_norm, all_wordlen_part_norm,
                                    all_valence_part_norm, all_imag_part_norm, all_m_list_part_norm,
                                    all_means_mpool_part_norm, all_trial_nos_part_norm, all_session_nos_part_norm))
...
print(multipletests([t_test_concreteness[1], t_test_wordfreq[1], t_test_wordlen[1], t_test_valence[1],
                      t_test_imag[1], t_test_mlist[1], t_test_mpool[1],
...
ave_betas = [round(np.mean(beta_concreteness), 3), round(np.mean(beta_wordfreq), 3), round(np.mean(beta_wordlen), 3),
            round(np.mean(beta_valence), 3) ,  round(np.mean(beta_imag), 3),
...
sem_betas = [round(stats.sem(beta_concreteness), 3), round(stats.sem(beta_wordfreq), 3),
             round(stats.sem(beta_wordlen), 3),  round(stats.sem(beta_valence), 3), round(stats.sem(beta_imag), 3),
...
predictors = ['Concreteness', 'Word Frequency', 'Word Length', 'Valence',
               'Imageability', 'List Meaningfulness', 'Pool Meaningfulness', "Trial No",
              "Session No", ]
...
plt.savefig("Final_List_Recall_NoArousalJan8.pdf")
plt.show()