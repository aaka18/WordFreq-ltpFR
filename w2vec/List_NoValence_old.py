...
def calculate_params():
    ...
    for i in range(len(all_probs)):
        ...
        x_values = np.column_stack((all_concreteness_part_norm, all_wordfreq_part_norm, all_wordlen_part_norm,
                                     all_arousal_part_norm, all_imag_part_norm,
...
sem_betas = [round(stats.sem(beta_concreteness), 3), round(stats.sem(beta_wordfreq), 3),
             round(stats.sem(beta_wordlen), 3),   round(stats.sem(beta_arousal), 3), round(stats.sem(beta_imag), 3),
...
plt.savefig("Final_List_Recall_NoValenceJan8.pdf")
plt.show()