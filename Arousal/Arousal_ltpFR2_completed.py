import glob
from Arousal.Arousal_Getbins import get_bins
import numpy as np
import matplotlib.pyplot as plt
from Arousal.Arousal_Excel import M
import scipy.io
        ...
        #test_mat_file = scipy.io.loadmat(f, squeeze_me=True, struct_as_record=False)
            with open(f, 'r') as jsfile:
                ...
                for id, bin in word_dict.items():
                    print(np.unique(word_dict.keys()))
                    print(pres_mat)
...
if __name__ == "__main__":
    ...
    plt.suptitle('Word Arousal', fontsize=20)
    ...
    plt.ylim(.40, .65)
    plt.savefig("WordArousalvsprec.Jan15.pdf")
    plt.show()
