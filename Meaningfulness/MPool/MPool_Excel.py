...
mpool_masked = np.array(mpool)[mask]
print(len(mpool_masked))
mpool_masked = np.sort(mpool_masked)
mpool_masked_final= (mpool_masked[:568])
print(mpool_masked_final)
# print(len(x_ltpFR))
...
print("Mean is = ", np.mean(mpool_masked_final))
print('Length of list of array_sorted:', len(mpool_masked_final))

bin_edges = []

# Finding bin edges by looking at the word pool in groups of 1/10 and identifying the max bin values
for i in range(1,21):
    edge = mpool_masked_final[int(i*len(mpool_masked_final)/20)-1]
...
for i, freq in enumerate(mpool_masked_final):
