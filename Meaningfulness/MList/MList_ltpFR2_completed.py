import glob
from Meaningfulness.MList.MList_Getbins import get_bins
import numpy as np
import matplotlib.pyplot as plt
from Meaningfulness.MList.MList_Excel import M
import scipy.io
...
if __name__ == "__main__":
    ...
    plt.suptitle('Word List Meaningfulness', fontsize=20)
    plt.xlabel('List Meaningfulness', fontsize=18)
    plt.ylabel('Recall Probability', fontsize=16)
    f_all_ltpFR2 = plt.errorbar(M, all_prob[0], yerr = sem_ltpFR_2[0])
    plt.ylim(.40, .65)
    plt.savefig("MListvsprec.Jan15.pdf")
    plt.show()
