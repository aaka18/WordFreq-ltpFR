...
# counting the number of occurences a word is presented
def count_presentations(a, pres_task):
    counts_with_task = []
    counts_no_task = []


    return np.array([len(np.where(pres_task > -1)[0]),len(np.where(pres_task == -1)[0])])



# counting the number of occurences a word is recalled
def count_recalls(recalled, pres_task):
    counts_with_task = []
    counts_no_task = []

    return np.array([len(np.where((pres_task > -1) & (recalled == 1))[0]), len(np.where((pres_task == -1) & (recalled == 1))[0])])
...
# sum count of all items presented to task and no task conditions
pres_counts = np.zeros(2)
# sum count of all items recalled to task and no task conditions
rec_counts = np.zeros(2)
...
for f in files_ltp:
    ...
    pres_counts += count_presentations(pres_mat, pres_task)
    rec_counts += count_recalls(rec_mat, pres_task)

print(prob(pres_counts, rec_counts))
...
#     participant_probs.append(probi)
#     all_prob += probi
#     n += 1
#
# all_prob /= n